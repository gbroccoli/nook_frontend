...
const setupSchema = (t: TFunction) => z.object({
  username: z.string()
...
const SetupForm = () => {
  ...
  const form = useForm({
    resolver: (data, context, options) =>
      zodResolver(setupSchema(t))(data, context, options),
  ...
  type FormValues = z.infer<ReturnType<typeof setupSchema>>
          ...
          <form className={"p-6 md:p-8"} onSubmit={form.handleSubmit(onSubmit)}>
            <FieldGroup>
              <div className={"flex flex-col items-center gap-2 text-center"}>
                <h1 className={"text-2xl font-bold font-pixel text-text"}>{t('auth.setup.title')}</h1>
                <p className={"text-muted-foreground text-balance"}>
                  {t('auth.setup.subtitle')}
                </p>
              </div>
              <Alert className={"border-primary/30 bg-primary/10 text-text"}>
                <ShieldCheck className={"text-primary"}/>
                <AlertTitle>{t('auth.setup.adminAlert.title')}</AlertTitle>
                <AlertDescription className={"text-text-secondary"}>
                  {t('auth.setup.adminAlert.description')}
                </AlertDescription>
              </Alert>
              <Controller
                name={"username"}
              ...
              <Controller
                ...
                render={({field, fieldState}) => (
                  <Field data-invalid={fieldState.invalid}>
                    ...
                    <Input {...field} id={"displayName"} placeholder={"Admin"}/>
                    {fieldState.error && (
